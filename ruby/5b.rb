...
books.each do |book|
  ...
  while !book_consistent?(book, rules_map)
    inconsistent_before = true
    # Sort using custom rules
    (0...(book.size - 1)).each do |i|
      page_1 = book[i]
      page_2 = book[i+1]
      if !rules_map[page_1].include?(page_2)
        book[i] = page_2
        book[i+1] = page_1
      end
    end
  ...
  acc += book[(book.size-1)/2] if inconsistent_before
end
